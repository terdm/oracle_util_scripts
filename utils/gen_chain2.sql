...
vNewCpcn_id  number;
vOldCpcn_id  number;
vStartDate   date := to_date('12.01.2022','DD.MM.RRRR');
vAddMonths   number := 3;
begin
    ...
    while i <= n loop 
        ...
        Insert into RISK_CORE.CORE_PRC_CHAIN_NODES_PARAMS
         ...
         Values
           ( vNewCpcn_id, 'F_START_DATE', to_char(vStartDate+i, 'DD.MM.RRRR'), 'N');
        
        Insert into RISK_CORE.CORE_PRC_CHAIN_NODES_PARAMS
           ( CPCN_ID, CPCNP_PARAM_NAME, CPCNP_PARAM_DATA, CPCNP_PARAM_IS_EXPR)
         Values
           ( vNewCpcn_id, 'F_END_DATE', to_char(vStartDate+i, 'DD.MM.RRRR'), 'N');   
